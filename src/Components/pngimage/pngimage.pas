  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        {Copies the new data}
        CopyMemory(Ptr(Longint(Output) + OutputSize - Longint(total_out)),
          @Buffer, total_out);
      ...
      begin
        Result := False;
        ErrorOutput := string(StreamRec.msg);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if avail_in = 0 then
        begin
          Result := Count - Longint(avail_out);
          Exit;
        end;
...
begin
  ...
  repeat
    {Copy this row}
    Dest^ := Owner.GammaTable[pByte(Longint(Src) + 4)^]; inc(Dest);
    Dest^ := Owner.GammaTable[pByte(Longint(Src) + 2)^]; inc(Dest);
    Dest^ := Owner.GammaTable[pByte(Longint(Src)    )^]; inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
    Extra^ := fOwner.GammaTable[pByte(Longint(Src) + 5)^]; inc(Extra);
    Extra^ := fOwner.GammaTable[pByte(Longint(Src) + 3)^]; inc(Extra);
    Extra^ := fOwner.GammaTable[pByte(Longint(Src) + 1)^]; inc(Extra);
    {$ENDIF}
...
begin
  ...
  repeat
    ...
    repeat
      ...
      {Copy data}
      Dest2^ := Dest2^ or
        ( ((Src^ shr CurBit) and BitTable[Header.BitDepth])
...
begin
  ...
  repeat
    ...
    Dest^ := fOwner.GammaTable[pByte(Longint(Src) + 4)^]; inc(Dest);
    Dest^ := fOwner.GammaTable[pByte(Longint(Src) + 2)^]; inc(Dest);
    Dest^ := fOwner.GammaTable[pByte(Longint(Src)    )^]; inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
...
begin
  ...
  begin
    ...
    begin
      {Writes this IDAT chunk}
      WriteIDAT(fStream, Data, Owner.MaxIdatSize - Longint(avail_out));
      {Re-update buffer}
      next_out := Data;
      avail_out := Owner.MaxIdatSize;
    end;

    if Longint(avail_out) < Owner.MaxIdatSize then
      {Writes final IDAT}
      WriteIDAT(fStream, Data, Owner.MaxIdatSize - Longint(avail_out));
...
begin
  ...
  repeat
    {Copy this row}
    Dest^ := fOwner.InverseGamma[pByte(Longint(Src) + 2)^]; inc(Dest);
    Dest^ := fOwner.InverseGamma[pByte(Longint(Src) + 1)^]; inc(Dest);
    Dest^ := fOwner.InverseGamma[pByte(Longint(Src)    )^]; inc(Dest);
...
begin
  ...
  repeat
    {Copy this row}
    Dest^ := Owner.InverseGamma[pByte(Longint(Src) + 2)^]; inc(Dest);
    Dest^ := Owner.InverseGamma[pByte(Longint(Src) + 1)^]; inc(Dest);
    Dest^ := Owner.InverseGamma[pByte(Longint(Src)    )^]; inc(Dest);
    Dest^ := Trans^; inc(Dest);
...
begin
  ...
  repeat
    ...
    begin
      dec(ChunkCount);
      Stream.Seek(Longint(ChunkLength + 4), soFromCurrent);
      Continue;
    end;
